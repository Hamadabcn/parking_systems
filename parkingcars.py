...
# Calculate the total cost based on the number of minutes
    if minutes <= 0:
        ...
        if remainder < 20:
            ...
            cost = days * price_3 + 20 * price_1 + 40 * \
                price_2 + (remainder - 60) * price_4
        elif remainder <= 454:
            cost = days * price_3 + 20 * price_1 + 40 * price_2 + \
                120 * price_4 + (remainder - 180) * price_5
        elif remainder < 1440:
            cost = (days + 1) * price_3
...
# If the user says yes, ask for the client information and format the receipt
    if receipt_needed.lower() == "y":
        ...
        receipt += f"Tel: +34 630 379 484\n" 
        receipt += f"e-mail: mohbcn1@hotmail.com\n"
